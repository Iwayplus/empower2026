...
import { sendOtp, verifyOtpSaveExhibitor } from '../../../services/api';
...
const OtpContainer = ({ type, input, onChangeNumber }) => {
  ...
  const handleVerifyOtp = async () => {
    ...
    let response = await verifyOtpSaveExhibitor({
    ...
    if (response?.status === 200) {
      ...
      navigate("/auth/exhibitor/register")
    }
    else {
      setError(true);
    ...
    >
      ...
      `}
          ...
          <p style={{ fontFamily: "Poppins", fontSize: "14px", fontWeight: 400, color: "#4A4A4A", margin: 0 }}>
            ...
            {timeLeft > 0 ? (
              <span style={{ fontWeight: 500 }}>{formatTime(timeLeft)}</span>
            ) : (
              <span
...
>
  ...
  `}</style>
            <h1 className="responsive-heading" aria-label="New Registration page heading">
            ...
            >
              ...
              <input
                ...
                style={{
                  ...
                  height: '100%',
                  padding: '0 12px',
                  paddingRight: error ? '40px' : '12px', // extra space for close icon
                  border: 'none',
                  outline: 'none',
                  color: 'var(--Color-Heading, #1C1C1C)',
...
`}
              ...
              {error && (
                <div
                  ...
                  onClick={() => {
                    // Optional: Reset input or error here
                    setInput('');
                    setError(false);
                  }}
                >
            ...
            >
              {error && (
                <p style={{ color: '#E76363', fontSize: '14px', margin: 0, fontFamily: 'Poppins', textAlign: 'left' }}>
                  {error}
                </p>
              )}
            </div>
            ...
            <button
              ...
              onMouseEnter={e => {
                if (!isLoading) e.currentTarget.style.background = '#4795EB'; // Hover
              }}
              onMouseLeave={e => {
                if (!isLoading) e.currentTarget.style.background = '#2180E4';
              }}
...
export default ExhibitorSignin;
